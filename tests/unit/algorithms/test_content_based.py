...
class TestContentBasedRecommendation:
    ...
    def test_tfidf_similarity_calculation(self, algorithm):
        ...
        assert np.all(
            (similarity_matrix >= -1e-10) & (similarity_matrix <= 1.0 + 1e-10)
        )
    ...
    async def test_get_user_preferences(self, algorithm):
        ...
        algorithm.interaction_repo.get_user_recommendation_data.assert_called_once_with(
            1
        )
    ...
    async def test_recommendation_generation_with_data(self, algorithm):
        ...
        algorithm._get_user_preferences = AsyncMock(
            return_value={
    ...
    async def test_recommendation_with_no_interactions(self, algorithm):
        ...
        algorithm.content_repo.get_trending_content = AsyncMock(
            return_value=[
    ...
    async def test_explain_recommendation(self, algorithm):
        ...
        algorithm.content_repo.get_content_with_stats = AsyncMock(
            return_value={
                "content": {
                    "id": 1,
