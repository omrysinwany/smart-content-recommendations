...
class TestHybridRecommendation:
    ...
    async def test_user_profile_analysis(self, algorithm):
        ...
        test_cases = [
            # New user
            {
                "input": {"total_interactions": 2},
                "expected_level": "minimal"
            },
            # Active user
            {
                "input": {"total_interactions": 15},
                "expected_level": "low"
            },
            # Power user
            {
                "input": {"total_interactions": 75},
                "expected_level": "high"
            }
        ]
    ...
    async def test_algorithm_combination(self, algorithm):
        ...
        content_3_score = next(score for content_id, score in combined if content_id == 3)
        content_1_score = next(score for content_id, score in combined if content_id == 1)
    ...
    async def test_fallback_mechanism(self, algorithm):
        ...
        result = await algorithm._get_fallback_recommendations(user_id=1, num_recommendations=5)
    ...
    async def test_full_recommendation_generation(self, algorithm):
        ...
        algorithm._analyze_user_profile = AsyncMock(return_value={
            "user_id": 1,
    ...
    async def test_recommendation_explanation(self, algorithm):
        ...
        algorithm.content_based.explain_recommendation = AsyncMock(return_value={
            "algorithm": "content_based",
            "confidence": 0.85,
            "reasons": ["Similar to your interests"]
        })
        
        algorithm.collaborative.explain_recommendation = AsyncMock(return_value={
...
class TestABTestingHybridRecommendation:
    ...
    def test_user_variant_assignment(self, ab_algorithm):
        ...
        valid_variants = ["control", "content_heavy", "collaborative_heavy", "trending_heavy"]
    ...
    async def test_variant_weight_application(self, ab_algorithm):
        ...
        ab_algorithm._analyze_user_profile = AsyncMock(return_value={
            "user_id": 1,
            "personalization_level": "medium"
        })
        ...
        ab_algorithm._get_algorithm_recommendations = AsyncMock(return_value=mock_results)
