...
class TestRecommendationService:
    ...
    async def test_recommendation_validation(self, service):
        ...
        with pytest.raises(ValidationError):
            await service._validate_recommendation_request(
                user_id=1,
                algorithm="hybrid",
                num_recommendations=100  # Exceeds max
            )
    ...
    async def test_recommendation_enrichment(self, service):
        ...
        # Test enrichment
        enriched = await service._enrich_recommendations(recommendation_result, user_id=1)
    ...
    async def test_trending_recommendations(self, service):
        ...
        service._enrich_recommendations = AsyncMock(return_value={
            "recommendations": [
    ...
    async def test_similar_content_recommendations(self, service):
        ...
        service.content_repo.get_content_with_stats = AsyncMock(return_value={
    ...
    async def test_recommendation_feedback_processing(self, service):
        ...
        valid_feedback_types = ["clicked", "liked", "dismissed", "not_interested", "reported"]
    ...
    async def test_recommendation_explanation(self, service):
        ...
        mock_algorithm = Mock()
        mock_algorithm.explain_recommendation = AsyncMock(return_value={
            "algorithm": "hybrid",
            "confidence": 0.85,
    ...
    async def test_error_handling(self, service):
        ...
        with pytest.raises(Exception):  # Service should re-raise for proper error handling
    ...
    async def test_caching_integration(self, service):
        ...
        service._validate_recommendation_request = AsyncMock()
        service._select_algorithm = AsyncMock(return_value=Mock(
            generate_recommendations=AsyncMock(return_value=RecommendationResult(
                content_ids=[1, 2], scores=[0.9, 0.8], algorithm_name="test", user_id=1
            ))
        ))
        service._enrich_recommendations = AsyncMock(return_value={
            "recommendations": [],
        ...
        # Call cached method
        result = await service.get_user_recommendations(user_id=1, num_recommendations=5)
