...
from sqlalchemy import String, ForeignKey, Float, Index, Enum as SQLEnum, UniqueConstraint
...
class Interaction(Base):
    ...
    # Interaction details
    interaction_type: Mapped[InteractionType] = mapped_column(SQLEnum(InteractionType), index=True)
    rating: Mapped[Optional[float]] = mapped_column(Float)  # 1-5 stars for ratings
    
    # Additional context (optional)
    interaction_metadata: Mapped[Optional[str]] = mapped_column(String(1000))  # JSON string for flexibility
    ...
    __table_args__ = (
        ...
        UniqueConstraint("user_id", "content_id", "interaction_type", name="unique_user_content_interaction"),
