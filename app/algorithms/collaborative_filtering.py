...
class CollaborativeFilteringRecommendation(BaseRecommendationAlgorithm):
        ...
        **kwargs
        ...
        try:
            ...
            user_interactions = await self.interaction_repo.get_user_interactions(
                user_id,
                interaction_types=[InteractionType.LIKE, InteractionType.SAVE, InteractionType.RATE]
            )
            ...
            self.logger.error(f"Error in collaborative filtering for user {user_id}: {e}")
    ...
    async def _user_based_recommendations(
        ...
        target_user_interactions = await self.interaction_repo.get_user_interactions(user_id)
        ...
        for similar_user in similar_users:
            ...
            similar_user_interactions = await self.interaction_repo.get_user_interactions(
                similar_user_id,
                interaction_types=[InteractionType.LIKE, InteractionType.SAVE, InteractionType.RATE]
            )
        ...
        metadata = {
            ...
            "avg_similarity": sum(u["similarity_score"] for u in similar_users) / len(similar_users),
            "total_candidates": len(content_scores),
            "recommender_details": {
                str(cid): content_recommenders[cid][:3]  # Top 3 recommenders per content
    ...
    async def _item_based_recommendations(
        ...
        metadata = {
            ...
            "total_candidates": len(content_scores),
            "similarity_details": {
                str(cid): content_similarity_details[cid][:3]
    ...
    async def _get_popular_fallback(
        ...
        trending_content_ids = await self.interaction_repo.get_trending_content_ids(
            days=30,  # Longer period for popular content
    ...
    async def explain_recommendation(
        ...
        try:
            if self.method == "user_based":
                return await self._explain_user_based_recommendation(user_id, content_id)
            else:
                return await self._explain_item_based_recommendation(user_id, content_id)
    ...
    async def _explain_user_based_recommendation(
        ...
        explanation = {
            ...
            "avg_similarity": (
                sum(u["similarity_score"] for u in recommending_users) / len(recommending_users)
                if recommending_users else 0
            ),
...
class MatrixFactorizationRecommendation(BaseRecommendationAlgorithm):
    ...
    async def explain_recommendation(self, user_id: int, content_id: int) -> Dict[str, Any]:
