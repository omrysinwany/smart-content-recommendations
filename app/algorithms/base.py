...
class RecommendationResult:
    ...
    def get_top_n(self, n: int) -> Tuple[List[int], List[float]]:
        ...
        sorted_pairs = sorted(
            zip(self.content_ids, self.scores),
            key=lambda x: x[1],
            reverse=True
        )
...
class BaseRecommendationAlgorithm(ABC):
    ...
    def calculate_diversity_score(self, content_ids: List[int], content_data: List[Dict]) -> float:
        """
    ...
    def apply_business_rules(
        ...
        max_age_days = user_profile.get("preferences", {}).get("max_content_age_days", 365)
