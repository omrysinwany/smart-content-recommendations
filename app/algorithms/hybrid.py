...
class HybridRecommendation(BaseRecommendationAlgorithm):
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.collaborative = CollaborativeFilteringRecommendation(
            db, method="user_based"
        )
        ...
        **kwargs,
        ...
        try:
            ...
            algorithm_results = await self._get_algorithm_recommendations(
                user_id, self.max_algorithm_candidates, exclude_content_ids, **kwargs
            )
            ...
            metadata = {
                ...
                "algorithm_contributions": {
                    algo: len(
                        [
                            r
                            for r in algorithm_results[algo].content_ids
                            if r in content_ids
                        ]
                    )
    ...
    async def _analyze_user_profile(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            user_data = await self.interaction_repo.get_user_recommendation_data(
                user_id
            )
            ...
            engagement_diversity = len(
                [k for k, v in interaction_summary.items() if v > 0]
            )
            ...
            profile = {
                ...
                "is_new_user": total_interactions
                < self.min_interactions_for_personalization,
    ...
    def _calculate_dynamic_weights(
        self, user_profile: Dict[str, Any], **kwargs
    ) -> Dict[str, float]:
        """
        ...
        **kwargs,
        ...
        try:
            ...
            results["collaborative"] = (
                await self.collaborative.generate_recommendations(
                    user_id, num_candidates, exclude_content_ids, **kwargs
                )
            )
        except Exception as e:
            self.logger.warning(f"Collaborative filtering algorithm failed: {e}")
            results["collaborative"] = RecommendationResult(
    ...
    async def _optimize_diversity(
        ...
        while len(optimized) < len(recommendations) and remaining:
            ...
            if best_item:
                ...
                selected_content_types.add(
                    ["article", "video", "course"][content_id % 3]
                )

        return optimized

    def _calculate_diversity_score(
        self, content_ids: List[int], content_metadata: List[Dict]
    ) -> float:
        """
        ...
        try:
            ...
            try:
                explanations["content_based"] = (
                    await self.content_based.explain_recommendation(user_id, content_id)
                )
            except Exception:
                pass
            ...
            try:
                explanations["collaborative"] = (
                    await self.collaborative.explain_recommendation(user_id, content_id)
                )
            except Exception:
                pass
...
class ABTestingHybridRecommendation(HybridRecommendation):
    ...
    def __init__(
        self, db: AsyncSession, experiment_config: Optional[Dict[str, Any]] = None
    ):
        super().__init__(db)
