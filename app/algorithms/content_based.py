...
class ContentBasedRecommendation(BaseRecommendationAlgorithm):
        ...
        **kwargs,
        ...
        try:
            ...
            candidate_content = await self._get_candidate_content(
                user_id, exclude_content_ids or []
            )
            ...
            scored_content = await self._calculate_content_similarities(
                user_profile, candidate_content
            )
    ...
    async def explain_recommendation(
        ...
        try:
            ...
            if similarity_breakdown["tag_score"] > 0.3:
                common_tags = set(user_profile.get("preferred_tags", [])) & set(
    ...
    def _calculate_similarity_scores(
        self, user_profile: str, content_features: List[str]
    ) -> List[float]:
    ...
    async def _build_user_profile(self, user_id: int) -> Dict[str, Any]:
        ...
        for interaction in positive_interactions:
            if interaction.interaction_type in [
                ...
                interaction.interaction_type == InteractionType.RATE
        ...
        profile["preferred_content_types"] = [
            ct for ct, _ in content_type_counts.most_common(3)
        ]
    ...
    def _calculate_recency_weight(self, interaction_date) -> float:
        ...
        if isinstance(interaction_date, str):
            interaction_date = datetime.fromisoformat(
                interaction_date.replace("Z", "+00:00")
            )
    ...
    async def _get_candidate_content(
        self, user_id: int, exclude_content_ids: List[int]
    ) -> List[Dict[str, Any]]:
        """
        ...
        return await self.content_repo.get_content_for_recommendations(
            exclude_user_id=user_id, exclude_content_ids=exclude_content_ids, limit=100
        )

    def _extract_content_features_single(
        ...
        features = {
            ...
            "tags": (
                content.get("content_metadata", {}).get("tags", [])
    ...
    async def _calculate_content_similarities(
        self, user_profile: Dict[str, Any], candidate_content: List[Dict[str, Any]]
    ...
    def _calculate_detailed_similarity(
        self, user_profile: Dict[str, Any], content_features: Dict[str, Any]
    ) -> Dict[str, float]:
        """
        Calculate detailed similarity breakdown.
        ...
        tag_score = self._calculate_jaccard_similarity(
            user_profile.get("preferred_tags", []), content_features.get("tags", [])
        )

        # Category similarity
        category_score = 0.0
        if content_features.get("category_id") and content_features[
            "category_id"
        ] in user_profile.get("preferred_categories", []):
            category_score = 1.0

        # Content type similarity
        content_type_score = 0.0
        if content_features.get("content_type") and content_features[
            "content_type"
        ] in user_profile.get("preferred_content_types", []):
            content_type_score = 1.0

        # Text similarity (simplified TF-IDF)
        text_score = self._calculate_text_similarity(user_profile, content_features)
    ...
    def _calculate_text_similarity(
        ...
        content_words = content_features.get("title_words", []) + content_features.get(
            "description_words", []
        )
    ...
    async def _get_trending_fallback(
        ...
        trending_content_ids = await self.interaction_repo.get_trending_content_ids(
            days=7, limit=num_recommendations
        )
