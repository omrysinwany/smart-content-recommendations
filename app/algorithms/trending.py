...
class TrendingRecommendation(BaseRecommendationAlgorithm):
        ...
        **kwargs
        ...
        try:
            ...
            if self.trending_type == "hot":
                return await self._get_hot_trending(user_id, num_recommendations, days_back, exclude_content_ids, **kwargs)
            elif self.trending_type == "rising":
                return await self._get_rising_trending(user_id, num_recommendations, days_back, exclude_content_ids, **kwargs)
            elif self.trending_type == "fresh":
                return await self._get_fresh_trending(user_id, num_recommendations, days_back, exclude_content_ids, **kwargs)
            elif self.trending_type == "viral":
                return await self._get_viral_trending(user_id, num_recommendations, days_back, exclude_content_ids, **kwargs)
            else:
    ...
    async def _get_rising_trending(
        ...
        **kwargs
        ...
        older_scores = {item["content_id"]: item["trending_score"] for item in older_data}
        ...
        # Normalize scores
        max_score = max([item["rising_score"] for item in top_rising]) if top_rising else 1
        ...
        metadata = {
            ...
            "avg_velocity": sum(item["velocity"] for item in top_rising) / len(top_rising) if top_rising else 0,
            "rising_details": [
                {
                    "content_id": item["content_id"],
    ...
    async def _get_fresh_trending(
        ...
        **kwargs
        ...
        metadata = {
            ...
            "avg_content_age_hours": sum(item["age_hours"] for item in top_fresh) / len(top_fresh) if top_fresh else 0,
            "fresh_details": [
                {
                    "content_id": item["content_id"],
    ...
    async def _get_viral_trending(
        ...
        **kwargs
        ...
        metadata = {
            ...
            "avg_viral_multiplier": sum(item["viral_multiplier"] for item in top_viral) / len(top_viral) if top_viral else 0,
            "viral_details": [
                {
                    "content_id": item["content_id"],
    ...
    async def _calculate_fresh_scores(
        ...
        **kwargs
        ...
        for item in trending_data:
            ...
            freshness_multiplier = math.exp(-content_age_hours / 24)  # Decay over 24 hours
    ...
    async def explain_recommendation(
        ...
        try:
            ...
            if self.trending_type == "hot":
                ...
                explanation["reasons"].append("Recently published with strong initial engagement")
                
            elif self.trending_type == "viral":
                explanation["reasons"].append("Showing exponential growth in shares and engagement")
            
            # Add engagement-based reasons
            if stats.get("engagement_rate", 0) > 0.1:
                explanation["reasons"].append(f"High engagement rate ({stats.get('engagement_rate', 0):.1%})")
            
            if stats.get("shares", 0) > 0:
                explanation["reasons"].append(f"Being actively shared ({stats.get('shares', 0)} shares)")
