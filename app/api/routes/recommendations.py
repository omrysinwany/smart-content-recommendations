...
async def get_user_recommendations(
    ...
    algorithm: str = Query("auto", description="Algorithm to use (auto, content_based, collaborative, hybrid, etc.)"),
    num_recommendations: int = Query(10, ge=1, le=50, description="Number of recommendations (1-50)"),
    exclude_content_ids: Optional[List[int]] = Query(None, description="Content IDs to exclude"),
...
async def get_trending_recommendations(
    trending_type: str = Query("hot", description="Type of trending (hot, rising, fresh, viral)"),
...
async def get_similar_content(
    ...
    num_recommendations: int = Query(10, ge=1, le=50, description="Number of similar items"),
...
async def get_system_health(
    ...
    **Health indicators:**
    ...
    try:
        ...
        return {
            "status": "healthy" if health_data["overview"]["status"] == "good" else "degraded",
        ...
        algorithm_info = {
            "available_algorithms": {
                "content_based": {
                    ...
                    "strengths": ["No cold start for items", "Explainable recommendations", "User independence"],
                    "limitations": ["Limited discovery", "Requires content features", "Filter bubble risk"],
                    "use_cases": ["New users with some interactions", "Content-rich platforms", "Niche interests"]
                },
                "collaborative": {
                    "name": "Collaborative Filtering",
                    "description": "Recommends content based on similar users' preferences",
                    "strengths": ["Discovers diverse content", "No content analysis needed", "Leverages community wisdom"],
                    "limitations": ["Cold start problem", "Sparsity issues", "Popularity bias"],
                    "use_cases": ["Active user base", "Social platforms", "Diverse content catalogs"]
                },
                "trending": {
                    ...
                    "limitations": ["Not personalized", "Popularity bias", "Short-term focus"],
                ...
                "hybrid": {
                    ...
                    "limitations": ["Complex", "Requires tuning", "Higher computational cost"],
                    "use_cases": ["Production systems", "Diverse user base", "Multiple content types"]
                }
            },
