...
class ContentCreateRequest(BaseModel):
    ...
    body: Optional[str] = Field(
        None,
        description="Content body (for articles, courses)"
    )
    url: Optional[HttpUrl] = Field(
        None,
        description="External URL (for videos, podcasts)"
    )
    ...
    tags: Optional[List[str]] = Field(
        default=[],
        description="Content tags for searchability"
    )
    ...
    is_featured: Optional[bool] = Field(
        default=False,
        description="Whether content should be featured"
    )
    ...
    def validate_body_for_content_type(cls, v, values):
        ...
        if content_type in [ContentTypeEnum.ARTICLE, ContentTypeEnum.COURSE]:
            if not v or len(v.strip()) < 100:
                raise ValueError(f'{content_type} must have substantial body content (min 100 characters)')
...
class ContentUpdateRequest(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        None,
        description="Updated content tags"
    )
    difficulty: Optional[DifficultyLevel] = Field(
        None,
