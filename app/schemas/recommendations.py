...
class RecommendationItem(BaseModel):
    ...
    content_type: str = Field(
        ..., description="Type of content (article, video, course, etc.)"
    )
    category: Optional[Dict[str, Any]] = Field(
        None, description="Content category information"
    )
    ...
    recommendation_score: float = Field(
        ..., ge=0, le=1, description="Recommendation confidence score (0-1)"
    )

    # Content statistics
    stats: Dict[str, Any] = Field(
        default_factory=dict, description="Content engagement statistics"
    )

    # User-specific context (if available)
    user_context: Optional[Dict[str, Any]] = Field(
        None, description="User-specific interaction context"
    )
...
class RecommendationFeedback(BaseModel):
    ...
    recommendation_id: Optional[str] = Field(
        None, description="Recommendation batch ID for tracking"
    )
...
class AlgorithmPerformance(BaseModel):
    ...
    click_through_rate: float = Field(
        ..., ge=0, le=1, description="Click-through rate (0-1)"
    )
    ...
    avg_relevance_score: float = Field(
        ..., ge=0, le=1, description="Average relevance score"
    )
    diversity_score: float = Field(
        ..., ge=0, le=1, description="Content diversity score"
    )
    coverage: float = Field(
        ...,
        ge=0,
        le=1,
        description="Catalog coverage (fraction of content recommended)",
    )
    novelty_score: float = Field(
        ...,
        ge=0,
        le=1,
        description="Novelty score (how often new content is recommended)",
    )
...
class OverallPerformance(BaseModel):
    ...
    avg_recommendations_per_user: float = Field(
        ..., description="Average recommendations per user"
    )
    user_engagement_rate: float = Field(
        ..., ge=0, le=1, description="Overall user engagement rate"
    )
    system_response_time_ms: float = Field(
...
class PerformanceAnalytics(BaseModel):
    ...
    algorithms: Dict[str, AlgorithmPerformance] = Field(
        ..., description="Per-algorithm performance metrics"
    )
...
class ExplanationDetail(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0, le=1, description="Confidence in this explanation"
    )
    reasons: List[str] = Field(
        ..., description="List of reasons for the recommendation"
    )
    user_factors: Optional[Dict[str, Any]] = Field(
        None, description="User factors that influenced the recommendation"
    )
    content_factors: Optional[Dict[str, Any]] = Field(
        None, description="Content factors that influenced the recommendation"
    )
...
class RecommendationExplanation(BaseModel):
    ...
    algorithm_explanations: List[ExplanationDetail] = Field(
        ..., description="Detailed explanations by algorithm"
    )

    # Content and user context
    content_details: Optional[Dict[str, Any]] = Field(
        None, description="Relevant content details"
    )
    user_context: Optional[Dict[str, Any]] = Field(
        None, description="Relevant user context"
    )
...
class OptimizationRecommendation(BaseModel):
    ...
    category: str = Field(
        ..., description="Optimization category (database, cache, system, etc.)"
    )
    ...
    estimated_improvement: str = Field(
        ..., description="Estimated improvement from this action"
    )
...
class OptimizationResult(BaseModel):
    ...
    optimization_completed: bool = Field(
    ...
    # Actions taken
    actions_taken: List[OptimizationAction] = Field(
        ..., description="List of optimization actions taken"
    )

    # Manual recommendations
    recommendations: List[OptimizationRecommendation] = Field(
        ..., description="Recommendations for manual optimization"
    )

    # Estimated improvements
    estimated_improvement: Dict[str, str] = Field(
        default_factory=dict, description="Estimated performance improvements"
    )
...
class AlgorithmCatalog(BaseModel):
    ...
    available_algorithms: Dict[str, AlgorithmInfo] = Field(
        ..., description="Available recommendation algorithms"
    )
    algorithm_selection_guide: Dict[str, List[str]] = Field(
        ..., description="Algorithm selection recommendations by use case"
    )
    performance_characteristics: Dict[str, Dict[str, str]] = Field(
...
class BatchRecommendationRequest(BaseModel):
    ...
    user_ids: List[int] = Field(
        ..., min_items=1, max_items=100, description="List of user IDs (1-100)"
    )
    algorithm: str = Field("auto", description="Algorithm to use for all users")
    num_recommendations: int = Field(
        10, ge=1, le=50, description="Number of recommendations per user"
    )
