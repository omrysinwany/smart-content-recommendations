...
class ContentService(BaseService):
    ...
    async def create_content(
        ...
        try:
            ...
            content_data = {
                ...
                "share_count": 0,
                "trending_score": 0.0,
                "quality_score": self._calculate_initial_quality_score(title, description, body)
            }
    ...
    async def get_content_detail(self, content_id: int, viewer_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        """
        self._log_operation("get_content_detail", content_id=content_id, viewer_id=viewer_id)
        ...
        try:
            ...
            if viewer_id and viewer_id != content.author_id:
                await self._record_interaction(viewer_id, content_id, InteractionType.VIEW)
            
            # Get similar content
            similar_content = await self.content_repo.get_similar_content(content_id, limit=5)
            
            # Get viewer interactions if viewer provided
            viewer_interactions = {}
            if viewer_id:
                viewer_interactions = await self._get_user_content_interactions(viewer_id, content_id)
    ...
    async def interact_with_content(
        ...
        """
        self._log_operation("interact_with_content", 
                          user_id=user_id, 
        ...
        try:
            ...
            if content.author_id == user_id and interaction_type in [InteractionType.LIKE, InteractionType.SAVE]:
            ...
            success = await self._record_interaction(user_id, content_id, interaction_type, rating)
            
            if success:
                # Update content statistics (in background in production)
                await self._update_content_stats_after_interaction(content_id, interaction_type)
    ...
    async def search_content(
        ...
        try:
            ...
            total_count = await self.content_repo.count({
                "is_published": True
            })
    ...
    async def get_trending_content(self, days: int = 7, limit: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        try:
            trending_content = await self.content_repo.get_trending_content(days, 0, limit)
    ...
    def _process_content_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "tags" in metadata:
            ...
            if isinstance(tags, list):
                processed["tags"] = [tag.strip().lower() for tag in tags[:10]]  # Max 10 tags
    ...
    async def _record_interaction(
        ...
        # This would typically use an InteractionRepository
    ...
    async def _get_user_content_interactions(
        ...
        # This would query the interactions table
