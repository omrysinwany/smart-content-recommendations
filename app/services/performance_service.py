...
class PerformanceMonitor:
    ...
    def track_api_call(self, endpoint: str, response_time: float, status_code: int):
        ...
        if response_time * 1000 > self.performance_thresholds["api_response_time_ms"]:
            self._create_alert("api_response_time", {
                "endpoint": endpoint,
    ...
    def track_database_query(self, query_type: str, execution_time: float):
        ...
        if execution_time * 1000 > self.performance_thresholds["db_query_time_ms"]:
            self._create_alert("slow_database_query", {
                "query_type": query_type,
                "execution_time_ms": execution_time * 1000,
    ...
    def track_recommendation_generation(self, algorithm: str, user_id: int, generation_time: float):
        """
    ...
    def get_performance_summary(self, hours_back: int = 1) -> Dict[str, Any]:
        ...
        recent_db_queries = [
            m for m in self.metrics["database_query_times"]
            if m["timestamp"] > cutoff_time
        ]
        ...
        if recent_db_queries:
            ...
            db_summary = {
                ...
                "slow_queries": len([q for q in query_times if q > self.performance_thresholds["db_query_time_ms"]])
            }
        ...
        if recent_resources:
            resource_summary = {
                "avg_cpu_percent": sum(m["cpu_percent"] for m in recent_resources) / len(recent_resources),
                "avg_memory_percent": sum(m["memory_percent"] for m in recent_resources) / len(recent_resources),
                "max_cpu_percent": max(m["cpu_percent"] for m in recent_resources),
                "max_memory_percent": max(m["memory_percent"] for m in recent_resources)
            }
    ...
    def get_bottleneck_analysis(self) -> Dict[str, Any]:
        ...
        if self.metrics["api_response_times"]:
            recent_api_calls = list(self.metrics["api_response_times"])[-100:]  # Last 100 calls
            avg_response_time = sum(m["response_time_ms"] for m in recent_api_calls) / len(recent_api_calls)
            ...
            if avg_response_time > self.performance_thresholds["api_response_time_ms"]:
                ...
                analysis["recommendations"].append("Consider implementing more aggressive caching")
        ...
        if cache_stats["hit_rate"] < self.performance_thresholds["cache_hit_rate"]:
            ...
            analysis["recommendations"].append("Implement cache warming for popular data")
        
        # Analyze database performance
        if self.metrics["database_query_times"]:
            recent_queries = list(self.metrics["database_query_times"])[-100:]
            slow_queries = [q for q in recent_queries if q["execution_time_ms"] > self.performance_thresholds["db_query_time_ms"]]
            ...
            if len(slow_queries) > len(recent_queries) * 0.1:  # More than 10% slow queries
                ...
                analysis["recommendations"].append("Add database indexes for slow queries")
...
class PerformanceService(BaseService):
    ...
    async def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            dashboard = {
                ...
                "recommendations": self._get_optimization_recommendations(summary, bottlenecks)
            }
    ...
    async def optimize_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(b["type"] == "database_performance" for b in bottlenecks["bottlenecks"]):
    ...
    def _analyze_cache_performance(self, cache_stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hit_rate >= 0.9:
            ...
            insights["optimization_opportunities"].append("Implement cache warming strategies")
        ...
        if total_ops > 1000:
            insights["observations"].append(f"High cache activity ({total_ops} operations)")
