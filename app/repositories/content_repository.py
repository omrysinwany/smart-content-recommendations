...
class ContentRepository(BaseRepository[Content]):
    ...
    async def update_content_stats(self, content_id: int) -> bool:
        ...
        stats_result = await self.db.execute(
            select(
                func.count(Interaction.id).filter(Interaction.interaction_type == InteractionType.VIEW).label('views'),
                func.count(Interaction.id).filter(Interaction.interaction_type == InteractionType.LIKE).label('likes'),
                func.count(Interaction.id).filter(Interaction.interaction_type == InteractionType.SAVE).label('saves'),
                func.count(Interaction.id).filter(Interaction.interaction_type == InteractionType.SHARE).label('shares')
            )
            .where(Interaction.content_id == content_id)
        )
    ...
    async def get_content_for_recommendations(
        ...
        query = query.order_by(desc(Content.trending_score), desc(Content.created_at)).limit(limit)
